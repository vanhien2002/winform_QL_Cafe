...
namespace BLL.Export
{
    public class ExcelExport
    {
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer, ExportThongKeModel model)
        {
            if (pReplacer != null)
            {
                pReplacer.Add("%NgayLap", model.ngayThongKe);
                pReplacer.Add("%tongDoanhThu", model.tongDoanhThu.ToString());
                pReplacer.Add("%tongSanPham", model.tongSP.ToString());  
                pReplacer.Add("%tongSoLuong", model.tongSoLuong.ToString());
            }
        }
        ...
        private bool OutSimpleReport<T>(List<T> dataSource, Dictionary<string, string> replaceValues, string viewName, bool isPrintPreview, ref string fileName)
        {
            ...
            // Get template stream
            string filepath = Directory.GetParent(AppDomain.CurrentDomain.BaseDirectory).Parent.Parent.FullName+ "\\Template\\BangThongKe.xls";
            MemoryStream stream = GetTemplateStream(filepath);
        ...
        /// <param name="viewName">Name of the view.</param>
        /// <returns></returns>
        private MemoryStream GetTemplateStream(string filepath)
        {
            ...
            //string filepath = Directory.GetParent(AppDomain.CurrentDomain.BaseDirectory).Parent.Parent.FullName+ "\\Template\\";
            arrByte = File.ReadAllBytes(filepath).ToArray();
            // Get stream
            if (arrByte.Count() > 0)
            {
        ...
        public bool ExportPhieuXuat(ExportThongKeModel dataSourceKH, List<exportSanPham> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            ...
            BuildReplacerCurrentDate(ref replacer, dataSourceKH);
